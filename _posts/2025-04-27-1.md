...
```
This results in an obvious UAF. Since we already have a write primitive, it's natural to think about overlapping freed objects with some useful kernel objects, like `seq_operations` or `tty_struct`, to overwrite function pointers and control RIP.

However, we cannot simply do it, since the objects are allocated from a dedicated cache. This is where cross-cache attack comes in.
