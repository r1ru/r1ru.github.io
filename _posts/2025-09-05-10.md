...
We can build this program using the `gcc` command. Here, we specify `-O0` to disable compiler optimizations, `-g` to enable debug information and `-no-pie` to disable [PIE (Position-independent Executable)](https://en.wikipedia.org/wiki/Position-independent_code):
```bash
...
file ./hello_world
```
The result shows that this file is in the [ELF(Executable and Linkable Format)](https://en.wikipedia.org/wiki/Executable_and_Linkable_Format). ELF is a file format widely used on Linux. An ELF file consists of three types of headers: the ELF header, section header, and program header, and consists of multiple sections and segments. The section header and program header store information about the sections and segments, respectively.
...
We can see that it contains information about the architecture, [ABI(Application Binary Interface)](https://en.wikipedia.org/wiki/Application_binary_interface), the program's entry point, and the offsets of the section and program header.
