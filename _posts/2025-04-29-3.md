...
In this post, I will explain DirtyCred, an attack technique that targets cred objects and is used in data-only attacks. Download the [handouts](https://github.com/r1ru/linux-kernel-exploitation/tree/main/dirty-cred) beforehand.
...
```
Note that `obj_alloc` internally uses `kmem_cache_zalloc`. It means a dedicated cache is used for managing these objects:
```c
...
```
This results in an obvious UAF vulnerability. By performing cross-cache attack explained in the [previous post](https://r1ru.github.io/posts/1), we can overlap victim objects with arbitrary kernel objects. However, we only have a read primitive, so it is difficult to control RIP.
