...
## Introduction
If we look at this chapter's `chal.c`, we'll notice that the `dump_stack` function has been removed:

![](./_assets/posts/16/0.jpg)
...
However, in `chal.c`, we are using glibc functions like `printf` and `putchar`. How is this possible?
